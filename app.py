...
<div class="card">
  ...
  <table>
    ...
    <tr class="{% if pl.cut %}tlb-cut-row{% endif %} tlb-row" data-tlb="{{ pl.name|lower }}">
      ...
      <td class="c tlb-thru hide-thru-mobile">{% if not pl.started %}-{% elif pl.thru and pl.thru > 0 and pl.thru < 18 %}{{ pl.thru }}{% elif pl.rounds and pl.rounds|length < 4 and pl.thru == 0 and pl.tee_detail %}<span style="color:rgba(255,215,0,.55);font-size:9px;white-space:nowrap;">{{ pl.tee_detail.replace(' ET','').replace(' AM','a').replace(' PM','p') }}</span>{% elif pl.thru == 18 or pl.thru == 0 %}F{% else %}-{% endif %}</td>
